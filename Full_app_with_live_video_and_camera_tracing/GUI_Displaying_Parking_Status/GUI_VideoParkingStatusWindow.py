...
from PyQt5.QtWidgets import (
    QMainWindow, QFileDialog, QVBoxLayout, QWidget, QPushButton, QLabel, QMessageBox, QComboBox
)
...
class VideoParkingStatusWindow(BaseWindow):
    ...
    def __init__(self):
        try:
            super().__init__()
            self.setWindowTitle("Video Üzerinden Park Alanı Analizi")
            ...
            # Başlık
            self.title_label = QLabel("- Video Üzerinden Park Alanı Analizi -", main_widget)
            self.title_label.setFont(QFont("Open Sans", 12, QFont.Bold))
            self.title_label.setObjectName("title")
            self.title_label.setAlignment(Qt.AlignCenter)
            self.title_label.setGeometry(300, 20, 500, 40)

            # Video görüntüleme QLabel (sağ tarafa taşındı)
            self.video_label = QLabel(main_widget)
            self.video_label.setGeometry(400, 100, 650, 500)
            self.video_label.setAlignment(Qt.AlignCenter)
            self.video_label.setStyleSheet("border: 2px solid #333533;")

            # Butonlar (sol tarafa hizalandı)
            self.select_video_button = QPushButton("Video Seç", main_widget)
            self.select_video_button.setFont(QFont("Open Sans", 10))
            self.select_video_button.setGeometry(50, 100, 300, 40)
            self.select_video_button.clicked.connect(self.select_video)

            self.video_status_label = QLabel("Henüz bir video seçilmedi.", main_widget)
            self.video_status_label.setGeometry(50, 160, 300, 40)
            ...
            self.credits_label.setGeometry(900, 660, 180, 20)
    ...
    def validate_video_aspect_ratio(self, selected_lot):
        """
        Videonun çözünürlüğünü kontrol eder ve yalnızca çözünürlük birebir eşleşirse doğrulamayı geçer.
        """
        try:
            if not self.video_path:
                QMessageBox.critical(self, "Hata", "Henüz bir video seçilmedi.")
                return False
            ...
            print(
                f"Original Resolution: {original_width}x{original_height}, Video Resolution: {video_width}x{video_height}")

            if original_width != video_width or original_height != video_height:
                QMessageBox.critical(
                    self,
                    "Çözünürlük Uyumsuzluğu",
                    f"Seçilen video çözünürlüğü ({video_width}x{video_height}), park alanı çözünürlüğü ({original_width}x{original_height}) ile eşleşmiyor. Lütfen uygun bir video seçin."
                )
                return False
            ...
            error_message = f"Hata oluştu: {e}\n{traceback.format_exc()}"
            print(error_message)
            QMessageBox.critical(self, "Hata", "Çözünürlük kontrolü sırasında bir hata oluştu.")
            return False
    ...
    def analyze_parking_status(self, frame, original_width, original_height, new_width, new_height):
        ...
        Video çerçevesi QLabel boyutlarına ölçeklenir.
        """
        try:
    ...
    def select_video(self):
        """
        Kullanıcıdan bir video seçmesini ister.
        """
        self.video_path, _ = QFileDialog.getOpenFileName(self, "Video Seç", "", "Video Dosyaları (*.mp4 *.avi *.mkv)")
        if self.video_path:
            self.video_status_label.setText(f"Seçilen Video: {os.path.basename(self.video_path)}")
            self.load_parking_button.setEnabled(True)
            QMessageBox.information(self, "Başarılı", "Video başarıyla seçildi.")
    ...
    def load_parking_data(self):
        ...
        """
        try:
            if VideoParkingStatusWindow.parking_data is None:
                # Firebase'den verileri yükleyin
                VideoParkingStatusWindow.parking_data = self.firebase.fetch_parking_lots()

            # ComboBox'u doldur
            self.parking_lot_combo.clear()
            self.parking_lot_combo.addItem("Park Alanı Seçin")
            for lot_name in VideoParkingStatusWindow.parking_data:
    ...
    def start_analysis(self):
        """
        Seçilen park alanına göre videoda analiz başlatır.
        """
        try:
            ...
            if selected_lot not in VideoParkingStatusWindow.parking_data:
            ...
            self.current_parking_spots = VideoParkingStatusWindow.parking_data[selected_lot].get("spots", [])

            # Video oranlarını doğrula
            if not self.validate_video_aspect_ratio(selected_lot):
                return
    ...
    def update_video_frame(self):
        """
        Videoyu oynatır ve park alanlarını analiz eder.
        """
        try:
            ...
            if not ret:
                self.timer.stop()
                self.cap.release()
                QMessageBox.information(self, "Bilgi", "Video sona erdi.")
                return
            ...
            # Video oranlarını koruyarak yeniden boyutlandır
    ...
    def finish_analyse(self):
        ...
        """
        try:
            # Eğer bir video oynatılıyorsa, durdur ve kaynakları serbest bırak
            ...
            self.video_status_label.setText("Henüz bir video seçilmedi.")
            ...
            QMessageBox.information(self, "Bilgi", "Analiz sona erdi. Yeni bir video ve park alanı seçebilirsiniz.")
    ...
    def closeEvent(self, event):
        ...
        try:
            if self.flag_for_GUI:
                # Eğer bir video açık ve oynatılıyorsa kaynakları serbest bırak
                if self.cap and self.cap.isOpened():
                    self.cap.release()
