...
class SavedParkingWindow(BaseWindow):
    ...
    def save_changes(self):
        ...
        selected_item = self.parking_list.currentItem()
        if not selected_item:
            QMessageBox.warning(self, "Hata", "Kaydetmek için bir park alanı seçmelisiniz.")
            return

        lot_name = selected_item.text()
        if lot_name not in self.parking_lots:
            QMessageBox.warning(self, "Hata", "Geçersiz park alanı.")
            return

        # Firebase'e güncellenmiş verileri yükle
        try:
            updated_data = self.parking_lots[lot_name]
            for i, spot in enumerate(updated_data.get("spots", [])):
                json_data = json.dumps(spot)
                temp_file_path = f"temp_parking_spot_{i + 1}.json"
                with open(temp_file_path, "w") as f:
                    f.write(json_data)

                cloud_path = f"parking_lots/{lot_name}/parking_spot_{i + 1}.json"
                self.firebase_operations.upload_file(temp_file_path, cloud_path)
                os.remove(temp_file_path)

            QMessageBox.information(self, "Başarılı", f"{lot_name} başarıyla güncellendi.")
